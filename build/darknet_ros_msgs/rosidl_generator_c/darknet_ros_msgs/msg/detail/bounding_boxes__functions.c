  ...
  darknet_ros_msgs__msg__BoundingBoxes__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    darknet_ros_msgs__msg__BoundingBoxes * data =
      (darknet_ros_msgs__msg__BoundingBoxes *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!darknet_ros_msgs__msg__BoundingBoxes__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          darknet_ros_msgs__msg__BoundingBoxes__fini(&data[i]);
        }
        free(data);
        return false;
      }
